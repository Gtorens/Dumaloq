...
const Atmosphere: React.FC = React.memo(() => {
    ...
    useEffect(() => {
        ...
        // Предзагрузка изображения для плавного появления
        const img = new Image();
        img.onload = () => {
            // Устанавливаем фон прямо через CSS-переменную для чистоты
            sectionElement.style.backgroundImage = `url('${img.src}')`;
        };
        img.src = '/images/atmosphere/gateway-mountains.jpg';

        // JS-параллакс для мобильных устройств (остается как fallback)
        const isMobile = window.matchMedia("(max-width: 768px)").matches;

        const handleScroll = () => {
            if (isMobile && sectionElement) {
                const top = sectionElement.getBoundingClientRect().top;
                // Двигаем фон медленнее, чем скролл, создавая эффект параллакса
                sectionElement.style.backgroundPosition = `center ${top * 0.3}px`;
            }
        };
    ...
    return (
        <section 
            ...
            // Используем только наш новый класс .parallax-bg
            // Он уже содержит все нужные свойства (bg-cover, bg-center и т.д.)
            className="parallax-bg relative h-[60vh] min-h-[500px] w-full flex items-center justify-center text-center text-white overflow-hidden section-spacing"
        >
